public class AEScipher {
  ...
  private static final int[][] GALOIS_CONST = { { 2, 3, 1, 1 }, { 1, 2, 3, 1 },
      { 1, 1, 2, 3 }, { 3, 1, 1, 2 } };
      ...
      for (int col = 0; col < 4; col++) {
        for (int row = 0; row < 4; row++) {
          if (keyChars.length == counter)
            ...
            keyArray[row][col] = Character.toString(keyChars[counter])
      ...
      for (int col = 0; col < 4; col++) {
        for (int row = 0; row < 4; row++) {
          if (keyChars.length == counter)
            ...
            plainTextArray[row][col] = Character.toString(keyChars[counter])
                + Character.toString(keyChars[counter + 1]);
            counter = counter + 2;
          }
        }
  ...
  /*
   * this function is called from the driver class It calculates the round keys
  ...
  /*
   * Calculates the xor of the hex values hexVal1 is the first hex value hexVal2
   * is the second hex value to be xored
   */
    ...
    try {
      ...
      for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 4; col++) {
          ...
          int xorResult = val1 ^ val2;
          String result = Integer.toHexString(xorResult);
          outHex[row][col] = result.length() == 1 ? ("0" + result) : result;
          // System.out.println(outHex[row][col]);
        }
      }
      return outHex;
    } catch (Exception ex) {
      System.out.println("Exception in aesStateXOR method: " + ex.getMessage());
      return null;
    }
  }

  /*
   * Substitutes input values with values from Sbox inStateHex: contains values
  ...
  protected String[][] aesNibbleSub(String[][] inStateHex) {
    ...
    try {
      ...
      System.out
          .println("Exception in aesNibbleSub method: " + ex.getMessage());
      return null;
    }
  }

  /*
   * Right Shifts values of every element of the matrix based on the shift value
   * inStateHex: input matrix on which shifting needs to be performed.
   */
  ...
  /*
   * Multiplies the 4x4 matrix with the Galios constants inStateHex: value that
   * needs to be multiplied with the Galios constant
   */
        ...
        for (int col = 0; col < 4; col++) {
          ...
          for (int counter = 0; counter < 4; counter++) {
            if (GALOIS_CONST[row][counter] == 1) {
              ...
              value = xor(value, multiply2(inStateHex[counter][col]));
            } else {
              String multiplyByTwoVal = xor(inStateHex[counter][col],
  ...
  /*
   * Performs left shift by 1 to give a value equivalent to multiplication by 2
   * Also if the MSB is 1 then it xor's the value with 1b
   */
  ...
  /*
   * Perform operations of adding key, nibble substitution row shift and mix
   * column based on the round number.
   * 
   * roundData:contains the input data roundKey:contains the key for each round
   * round:contains the round number
   */
  protected String[][] calcEachRound(String[][] roundData, String[][] roundKey,
      int round) {
    try {
      String[][] output = null;
  ...
  private void print(String[][] encrytedText) {
    try {
      ...
      System.out.println("Exception in print() : " + ex.getMessage());
    }
  }
}
